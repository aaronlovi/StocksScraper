...
internal sealed class GetGrahamScoreSnapshotsStmt : QueryDbStmtBase {
    ...
    public GetGrahamScoreSnapshotsStmt(DateOnly asOfDate, PaginationRequest pagination, ScoresFilter? filter)
        : base(BuildSql(filter), nameof(GetGrahamScoreSnapshotsStmt)) {
        _asOfDate = asOfDate;
    ...
    private static string BuildSql(ScoresFilter? filter) {
        var whereClauses = new List<string> { "as_of_date = @as_of_date" };
...
{whereClause}
ORDER BY overall_score DESC, computable_checks DESC, company_id ASC
