...
internal class RawDataGrpcService : RawDataServiceBase
{
    ...
    {
        if (request is null) return FailureAsWarning("Request is null");
        if (context is null) return FailureAsWarning("Context is null");

        try
        {
            using var reqIdLogContext = _logger.BeginScope("RequestId: {RequestId}", request.RequestId);
