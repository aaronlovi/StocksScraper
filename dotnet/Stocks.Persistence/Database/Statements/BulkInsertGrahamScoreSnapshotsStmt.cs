...
internal sealed class BulkInsertGrahamScoreSnapshotsStmt : BulkInsertDbStmtBase<CompanyScoreSummary> {
    private readonly DateOnly _asOfDate;

    public BulkInsertGrahamScoreSnapshotsStmt(DateOnly asOfDate, IReadOnlyCollection<CompanyScoreSummary> scores)
        : base(nameof(BulkInsertGrahamScoreSnapshotsStmt), scores) {
        _asOfDate = asOfDate;
    }

    protected override string GetCopyCommand() => "COPY graham_score_snapshots"
        + " (as_of_date, company_id, cik, company_name, ticker, exchange,"
    ...
    protected override async Task WriteItemAsync(NpgsqlBinaryImporter writer, CompanyScoreSummary s) {
        await writer.WriteAsync(_asOfDate.ToDateTime(TimeOnly.MinValue), NpgsqlDbType.Date);
